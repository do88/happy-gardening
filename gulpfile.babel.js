 ...
 * Task: `webpackJS`.
 *
 * Concatenate and run JS through webpack
 */
gulp.task("webpackJS", () => {
	return gulp
		.src(config.watchJsAll, { since: gulp.lastRun("webpackJS") }) // Only run on changed files.
		.pipe(plumber(errorHandler))
...
gulp.task(
	...
	gulp.parallel("styles", "webpackJS", "images", browsersync, () => {
		...
		gulp.watch(config.watchJsAll, gulp.series("webpackJS", reload)); // Reload on customJS file changes.
